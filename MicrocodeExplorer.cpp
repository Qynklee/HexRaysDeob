...
public:
	...
	bool Display(minsn_t *top, sample_info_t *si, int nBlock, int nSerial)
	{
		...
		m_Title.cat_sprnt("Microinstruction Graph - %llu[%s]/%d:%d", (uint64_t)mba->entry_ea, MicroMaturityToString(si->mat), nBlock, nSerial);
		m_TW = create_empty_widget(m_Title.c_str());
		netnode id;
		id.create();
		
		m_GVName.cat_sprnt("microins_%llu_%s_%d_%d", (uint64_t)mba->entry_ea, MicroMaturityToString(si->mat), nBlock, nSerial);
...
public:
	...
	{
		mbl_array_t *mba = *si->mba;
		m_Title.cat_sprnt("Microcode Graph - %llu[%s]", (uint64_t)mba->entry_ea, MicroMaturityToString(si->mat));

		TWidget *tw = create_empty_widget(m_Title.c_str());
		netnode id;
		id.create();
		
		m_GVName.cat_sprnt("microblkgraph_%llu_%s", (uint64_t)mba->entry_ea, MicroMaturityToString(si->mat));
...
void ShowMicrocodeExplorer()
{
	...
	if (mba == NULL)
	{
		warning("#error \"%llu: %s", (uint64_t)hf.errea, hf.desc().c_str());
		return;
	}
	...
	qstring title;
	title.cat_sprnt("Microcode Explorer - %llu - %s", (uint64_t)pfn->start_ea, MicroMaturityToString(mmat));
